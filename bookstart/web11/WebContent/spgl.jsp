				...
				<div class="col-lg-10">
					...
					<spn style="color:red;  display:inline-block; margin-left:800px;"><strong>${sshibai}</strong></spn>
						...
						<div class="row" style="margin-left: 500px">
						 ...
						 <c:if test="${Allproduct.getDangqianpage()!=1}">
						 <li><a href="${pageContext.request.contextPath}/producthoutai?dangqianpage=${Allproduct.getDangqianpage()-1}&ishod=${vobean.getIshod()}&shangpinleibie=${vobean.getShangpinleibie()}&shangpinname=${vobean.getShangpinname()}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
						</c:if>
						
						<c:forEach begin="1" end="${Allproduct.getZongpage()}" var="var">
						<c:if test="${Allproduct.getDangqianpage()==var}">
						<li class="active">
						<a href="${pageContext.request.contextPath}/producthoutai?dangqianpage=${var}&ishod=${vobean.getIshod()}&shangpinleibie=${vobean.getShangpinleibie()}&shangpinname=${vobean.getShangpinname()}">${var}<span class="sr-only">(current)</span></a></li>
						</c:if>
						<c:if test="${Allproduct.getDangqianpage()!=var}">
						<li>
						<a href="${pageContext.request.contextPath}/producthoutai?dangqianpage=${var}&ishod=${vobean.getIshod()}&shangpinleibie=${vobean.getShangpinleibie()}&shangpinname=${vobean.getShangpinname()}">${var}<span class="sr-only">(current)</span></a></li>
						 ...
						 <c:if test="${Allproduct.getDangqianpage()!=Allproduct.getZongpage()}">
						 <li><a href="${pageContext.request.contextPath}/producthoutai?dangqianpage=${Allproduct.getDangqianpage()+1}&ishod=${vobean.getIshod()}&shangpinleibie=${vobean.getShangpinleibie()}&shangpinname=${vobean.getShangpinname()}" aria-label="Next">
						 <span aria-hidden="true">&raquo;</span>
						 </a></li>
						</c:if>
						     </ul>
